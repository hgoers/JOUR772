...
**A1.** https://datawrapper.dwcdn.net/YlU8N/1/

```{r}
md_gov_county <- rio::import("data/md_gov_county.csv")
md_gov_county_18 <- rio::import("data/md_gov_county_18.csv") |> 
  select(-County) |> 
  rename_with(~ paste0(.x, "_18"), .cols = -GEOID)

md_gov_county_comp <- md_gov_county |> 
  left_join(md_gov_county_18, by = "GEOID") |> 
  janitor::clean_names() |> 
  rowwise() |> 
  mutate(total = sum(across(cox:write_ins)),
         total_18 = sum(across(hogan_18:write_ins_18)),
         perc_cox = cox / total * 100,
         perc_hogan = hogan_18 / total_18 * 100)

rio::export(md_gov_county_comp, "data/md_gov_county_comp.csv")
```
...
**A2.** https://app.datawrapper.de/chart/jfFdg/publish

Mail-in voting is yet to return to pre-pandemic levels. However, I would probably want to see more years' data to get a better sense of the actual baseline level of mail-in voting. 

```{r}
md_votes_by_type <- rio::import("data/md_votes_by_type.csv") |> 
  select(Year, Early = pct_early, `Election day` = pct_election_day, Mail = pct_mail)

rio::export(md_votes_by_type, "data/md_votes_by_type_cleaned.csv")
```
...
**A3.** https://datawrapper.dwcdn.net/Qreuj/2/

Bigger rural zipcodes with over $20,000 in donations. Who lives here? Are these many small donors or single large donors? Are these in swing counties? 

```{r}
md_actblue_zip <- rio::import("data/md_actblue_zip.csv") |> 
  mutate(zip = case_when(zip == 2116L ~ 21160L,
                         zip == 2130L ~ 21300L,
                         TRUE ~ zip),
         grand_total = sum(total),
         perc = total / grand_total * 100)

rio::export(md_actblue_zip, "data/md_actblue_zip_clean.csv")
```
