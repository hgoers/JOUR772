...
session <- bow(url, user_agent = "Harriet Goers, hgoers@umd.edu")
session
```

```{r}
results_2022_raw <- scrape(session) |> 
  html_table()

results_2022 <- results_2022_raw[[1]] |> 
  row_to_names(row_number = 1) |> 
  pivot_longer(`ABRAHAM KORB (Write-In)`:`TOMMY WILLIAMS (IAP)`, names_to = "candidate", values_to = "count") |> 
  clean_names() |> 
  filter(county != "Total") |> 
  mutate(year = 2022,
...
```{r}
results_2018 <- rio::import("https://elections.utah.gov/Media/Default/2018%20Election/2018%20General%20Election%20Canvass.xlsx", sheet = "U.S. Senate") |> 
  row_to_names(row_number = 2) |> 
  pivot_longer(`Tim Aalders        (CON)`:`Tyrone Jensen         (Write-in)`, names_to = "candidate", values_to = "count") |>
  clean_names() |> 
  filter(!county %in% c("TOTAL", "Percentage", "Contest Sum")) |> 
  mutate(year = 2018,
         count = as.numeric(count))
...
```{r}
results <- results_2018 |> 
  bind_rows(results_2022)
...
```{r}
results |> 
  filter(candidate %in% c("Jenny Wilson              (DEM)", "EVAN MCMULLIN (UNA)")) |> 
  arrange(county, year) |>
  group_by(county) |> 
...
```{r}
results |> 
  filter(candidate %in% c("Mitt Romney              (REP)", "MIKE LEE (REP)")) |> 
  arrange(county, year) |>
  group_by(county) |> 
  mutate(diff = count - lag(count)) |> 
  arrange(diff)
...
**A3** I might run sentiment analysis and topic modelling on the titles and look at how these change over the course of the campaign and which topics his campaign positively emphasises.  
...
```{r}
pull_article <- function(url) {
  
  session <- bow(url, user_agent = "Harriet Goers, hgoers@umd.edu")
  
  article_text <- scrape(session) |>
    html_node(xpath = "//*[@id='press-releases_blog']/div/div[4]") |>
    html_text()
  
  print(glue::glue("Completing { url }"))
  
  return(article_text)

}

article_texts <- map(cardin_releases$url, ~ pull_article(.x))

full_df <- cardin_releases |> 
  mutate(text = unlist(article_texts))
```
