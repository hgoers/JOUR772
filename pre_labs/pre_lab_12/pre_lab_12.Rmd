...
get_new_rego_by_party <- function(month, year) {
  ...
  report_url <- glue::glue("https://elections.maryland.gov/pdf/vrar/{ year }_{ month }.pdf")
...
get_removals_by_party <- function(month, year) {
  ...
  removals <- table[[1]] |> 
    ...
    clean_names() |> 
    slice(-(1:2)) |> 
    rename(method = x13, DEM = x14, REP = x15, GRN = x16, LIB = x17, WCP = x18, UNA = x19, OTH = x20) |> 
    transmute(method, 
              across(DEM:OTH, ~ as.numeric(str_remove_all(.x, ","))),
              month = month,
              year = year) |> 
...
**Answer** Two different tables stored in a list object. 

```{r}
tables <- extract_tables("https://elections.maryland.gov/pdf/vrar/2022_10.pdf")
View(tables[[1]])
```
...
**Answer** We get a table of new registrations by party. 
...
new_registrations_1 <- get_new_rego_by_party(10, 2022)
...
**Task** Run the following code and describe the result.
**Answer** We get a table of removals by party and process. 
...
removals_1 <- get_removals_by_party(10, 2022)
...
**Answer** The sheer difference in the number of new registered voters for the Dems compared to other parties, including the Republicans. 
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** Most unaffiliated voters are removed through other means. This is not the case for the major parties. 
...
**Answer** 16 rows compared to 17 rows. 
...
**Answer** Unaffiliated individuals are the only ones increasingly using absentee ballots between these two periods. 
...
**Answer** Shows the percent of democrat registrations by type. The ERIC report seems to be the most popular option. 
...
**Answer** In-person is much more popular in October than in June. 
