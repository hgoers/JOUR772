...
md10 |> 
  ...
  mutate(name = str_remove(name, " County, Maryland"),
         year = factor(year)) |> 
  ggplot(aes(x = estimate, y = reorder(name, estimate), group = year)) +
  geom_errorbarh(aes(xmin = estimate - moe, xmax = estimate + moe, colour = year)) +
...
**Answer** None outside the margin of error. 
...
top_zips <- md_contribs %>%
  group_by(zip5) %>%
  summarise(total_contributed = sum(contribution_receipt_amount)) %>%
  ungroup()
```
...
```{r}
md_zips_with_acs <- get_acs(geography="zcta", 
                       variables = "B19013_001", 
                       state = "MD", 
                       year = 2019) |> 
  right_join(top_zips, by = c("GEOID" = "zip5")) |> 
  arrange(desc(estimate))

head(md_zips_with_acs)
```

**Answer** Of the 20 zip codes with the largest amounts, only one has an estimated median household income of less than $50,000: `r md_zips_with_acs |> arrange(desc(total_contributed)) |> head(20) |> filter(estimate < 50000) |> pull(GEOID)`.
...
```{r}
top_10_avg <- md_zips_with_acs |> 
  head(10) |> 
  summarise(mean(estimate)) |> 
  pull()

avg <- md_zips_with_acs |> 
  summarise(mean(estimate, na.rm = T)) |> 
  pull()

contrib <- md_zips_with_acs |> 
  mutate(top_10 = if_else(GEOID %in% (md_zips_with_acs |> head(10) |> pull(GEOID)), 1, 0)) |> 
  group_by(top_10) |> 
  summarise(contrib = mean(total_contributed)) |> 
  mutate(prop = contrib / lag(contrib)) |> 
  drop_na() |> 
  pull(prop)
```

**Answer** The average median income of the ten richest zipcodes in Maryland is `r scales::dollar(top_10_avg)`. This is `r round(top_10_avg/avg, 2)` times the state-wide average of `r scales::dollar(avg)`. This is not reflected in the proportional total contributions provided by this group: those who live in the top ten richest zipcodes contributed `r round(contrib, 2)` times the amount provided by all other zipcodes to the Republican party. What are the demographics of those living in these areas? Why are they willing to provide a disproportionately large amount to the Republican party? Are these trends consistent for Democrats? 