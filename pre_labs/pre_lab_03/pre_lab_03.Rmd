...
**Answer** Warning: One or more parsing issues, see `problems()` for detailsRows: 476915 Columns: 13── Column specification.
...
**Answer** There were 1,640 problems. There are mixed types of data in several columns. 
...
**Answer** No warnings or errors provided. 
...
**Answer** The `election_day` field is a character despite the values looking numeric. 
...
texas_precinct_20 <- texas_precinct_20 %>%
  mutate(election_day = if_else(election_day == '5+', 49, as.numeric(election_day)))
...
**Answer**: 402,345 results list mail as NA. This represents `r round(402345/476915 * 100, 2)` percent of all results. To understand mail-in voting at the state level, we would need to uncover what NA represents. If NA is meaningful and consistently recorded across precincts, we can use these data to answer our questions. If not, we need to deal with this missingness. We could listwise delete the rows that including missing data, or we could try to impute the missing data from other, similar precincts that have mail data.   
...
**Answer** There are 12. registrations in the data for January 1900. The first two results suggest some data input error: these dates are simply too early to be believable. The other 18 results are more plausible. They suggest a few very old voters in Yadkin.   
...
**Answer** A large number of voters are listed as REMOVED AFTER 2 FED GENERAL ELECTIONS IN INACTIVE STATUS. Thousands more appear to be at risk of being removed from the voter roll either because they have not returned their confirmation, or because their confirmations was returned after being marked as undeliverable. It would be interesting to see this process of removal and whether it appropriately reflects the issues the county is attempting to resolve or if, in fact, it disenfrancises groups within the county. 
...
1.    The amount column is stored as a character, not a number.

2.    The city field has five different forms – including misspellings – of Arnold.

3.    The zip field mixes five digit ZIP codes and nine digit ZIP codes.
...
We have asked the code to do a calculation using data that is in a format that does not allow for this calculation. We need this variable to be numeric, not character.  
...
**Answer** There is one set of duplicates in the data. The donor is Derrick Hamilton. 
...
**Answer** The ZIP code is now consistently the first five digits of whatever number was supplied. If inputted correctly, this should reflect the five digit ZIP code. 
...
**Answer** There are two misspellings of Conowingo: Conowing and Conowingoo. 
...
# display the cleaned dataset
distinct(cleaned_conowingo, city)
```
