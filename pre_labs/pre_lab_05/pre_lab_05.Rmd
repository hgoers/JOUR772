...
**Answer** Different spelling for the same recipient. 
...
**Answer** JOE'S SEAFOOD PRIME STEAK & STONE CRAB to JOE'S SEAFOOD, PRIME STEAK & STONE CRAB, and LE DIPLOMATE, LE DIPLOMATE to LE DIPLOMATE. 

```{r}
cleaned_dc_food <- dc_food %>%
  mutate(recipient_clean=key_collision_merge(recipient_name)) %>%
  relocate(recipient_name, recipient_clean)

cleaned_dc_food |> 
  filter(recipient_name != recipient_clean)
```
...
**Answer** W. MILLAR & CO to W. MILLAR & CO., WALTER'S SPORTS BAR to WALTERS SPORTS BAR, HILTON WASHINGTON to WASHINGTON HILTON. 
...
**Answer** These methods largely result in the same cleaned names. Only two percent of all names are different. 

```{r}
dc_food |> 
  mutate(recipient_clean_kc = key_collision_merge(recipient_name), 
         recipient_clean_ng = n_gram_merge(recipient_name),
         kc_ng_same = if_else(recipient_clean_kc == recipient_clean_ng, 1, 0)) |> 
  group_by(kc_ng_same) |> 
  summarise(n = n()) |> 
