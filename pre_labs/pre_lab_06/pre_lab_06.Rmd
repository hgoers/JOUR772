...
```{r}
voters_by_county <- read_csv("data/tabula-Eligible Active Voters by County - GP22.csv")
...
**Answer** 
- No variable label is provided for `x1`
- A row has been added with the note "Jurisdiction Wide".
- `voters` and `turnout` are read as characters.
- Totals are provided, making summaries more difficult to calculate. 
- A jurisdiction name runs over two rows. 
...
- A row has been added with the note "Jurisdiction Wide".
- Totals are provided, making summaries more difficult to calculate. 
- A blank row remains with the end of a precinct name. 
