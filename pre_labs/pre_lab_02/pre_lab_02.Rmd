...
```{r}
primary_18 <- read_csv(here::here("pre_labs", "pre_lab_02", "data", "maryland_primary_2018.csv"))
```
...
**Answer** This code creates a new column, `percent_election_day`, that holds the proportion of the total number of votes that were made on election day.  
...
**Answer** The dataframe is now ordered by `percent_election_day`. No values changed. This is a more meaningful order. 
...
**Answer** Yes, it shows the candidates who received the highest proportion of votes not on election day. This may indicate that they receive strong support from people who are more likely to vote by mail or early (for example, young people, military personnel, people living overseas). 
...
`str_to_sentence` capitalises the first character and makes every following character lowercase. 

`str_to_lower` makes all characters lowercase. 
...
**Answer** If we do not create a new object, we cannot access these manipulations without running the code again.
...
**Answer** You have binned the data by month-year. For example, any date that falls within the month of June 2022 is recoded to 1 June 2022. This means that we can consider the values by month, instead of by day. 
